...
- [x] GET /api/v1/auth/discord
  - ログイン画面にリダイレクトする
- [x] GET /api/v1/auth/discord/callback
  - 認証後の情報をサーバで処理するコールバックエンドポイント
  - API用トークンとリフレッシュトークンを返す
- [x] GET /api/v1/auth/refresh-token
  - リフレッシュトークンを受け取り、問題なければAPIトークンを返す
- [ ] POST /api/v1/tsumikis
  - 積み木＆1つ目のブロック作成
  - メディアのリレーションの紐づけはこのエンドポイントでやる
...
- [ ] POST /api/v1/tsumikis/{tsumiki_id}/medias
  - メディアのアップロード受付、S3横流し、DB上のデータ作成
  - ファイルサイズ上限
    - 動画：100MB、3分以内
    - 音声：5MB、5分以内
    - 画像：5MB、4096x4096以内
  - スコープ長め、リミット重めのレートリミットをかける
    - 1日で10件
  - 使わなかったメディアはS3に残るので、バッチでの清掃が必須
- [ ] GET /api/v1/works
  - 作品一覧取得
